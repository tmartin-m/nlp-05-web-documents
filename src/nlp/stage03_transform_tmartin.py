...
def run_transform(
    ...
    LOG.info("========================")
    LOG.info("STAGE 03e: Extract PDF link")
    LOG.info("========================")

    pdf_link_tag: Tag | None = soup.find(
        "a",
        title="Download PDF",
    )

    if pdf_link_tag and pdf_link_tag.get("href"):
        pdf_link: str = f"https://arxiv.org{pdf_link_tag['href']}"
    else:
        LOG.warning("PDF link not found, setting to 'unknown'")
        pdf_link: str = "unknown"

    LOG.info(f"Extracted pdf_link: {pdf_link}")

    LOG.info("========================")
    LOG.info("STAGE 03f: Calculate derived fields")
    LOG.info("========================")
